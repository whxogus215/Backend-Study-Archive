...
## 쿠키
HTTP는 무상태(Stateless) 프로토콜이기 때문에, 클라이언트와 서버의 연결이 끊어지면 서로의 상태를 유지하지 않는다.
따라서 클라이언트가 다시 요청을 하면 서버는 기억하지 못하기 때문에 인증정보 등이 저장되지 않는다.  

그렇다면 클라이언트가 요청하는 Path에 사용자 정보를 포함해서 전달한다면 어떻게 될까? 일단 보안상의 이슈는 당연하며,
매번 정보를 담아서 보내야 하는 클라이언트 입장에서도 번거로울 것이다. 통신 속도에도 영향을 줄 것이다. 또한 사용자 정보를
요청에 포함도록 개발해야 한다.

- Set-Cookie : 서버 측에서 클라이언트로 쿠키를 전달하는 응답헤더 필드이다.
- Cookie : 클라이언트가 서버에서 받은 쿠키를 저장하고, HTTP 요청 시에 서버 측으로 전달하는 요청헤더 필드이다.

![쿠키 그림1](https://github.com/whxogus215/Backend-Study-Archive/assets/70999462/6b26f07e-a47c-4939-ba24-b80f17868547)
서버 측에서 응답할 때, 사용자 정보에 대한 내용을 쿠키에 저장하여 보낸다. (Set-Cookie 헤더 필드) 그러면 클라이언트는
전달받은 쿠키 정보를 통해 Key와 Value 값을 웹 브라우저의 쿠키 저장소에 저장한다.
![쿠키 그림2](https://github.com/whxogus215/Backend-Study-Archive/assets/70999462/fbf4be98-3d3f-4165-9e8f-e9053a2f412d)
클라이언트는 요청할 때, 먼저 쿠키를 뒤진다. 그리고 해당 요청에 맞는 쿠키 정보가 존재하면 Cookie라는 헤더 필드에 값을 저장하여 요청한다.
URI에 사용자 정보를 넣지 않아도 된다.
![쿠키 그림3](https://github.com/whxogus215/Backend-Study-Archive/assets/70999462/e6eb3789-2586-42dd-8fd4-50e1d6d223b5)
쿠키는 요청할 때마다 모든 요청에 자동으로 포함된다.

서버 측에서 저장하는 쿠키의 예시) set-cookie : **sessionId=abcde1234;** expires=Sat, 26-Dec-2020 00:00:00 GMT; path=/; domain=.google.com; Secure
**쿠키에 저장하는 값은 세션 ID이다.** 즉, 사용자에 관한 정보를 직접 저장하지 않고, 일종의 ID를 DB에 저장해놓았다가 이를 쿠키에 저장한다.

쿠키는 항상 자동으로 서버 측으로 포함되어 전송되기 때문에 쿠키의 개수가 많을 경우, 쿠키가 담고있는 정보가 많을 경우 네트워크에 추가적인 트래픽이 생길 수 있다.
따라서 **쿠키에는 최소한의 정보(세션 ID, 인증 토큰)만 담겨야 한다.**  

만약 매번 서버에 쿠키를 보내지 않고, 클라이언트 측에서 자체적으로 쿠키를 활용하는(자바 스크립트) 방법으로 웹 스토리지(localStorage, sessionStorage)를 사용할 수 있다.
그러면 서버 측으로 보내는 쿠키의 양이 줄어들 수 있다.

**또한 쿠키에는 민감한 정보가 들어가선 절대 안된다! ex) 주민번호, 신용카드 번호 등**

### 쿠키 생명주기
1. 세션 쿠키 : 만료 날짜를 지정하지 않으면 브라우저 종료할 때까지만 쿠키가 유지된다.
   - 브라우저를 종료했다가 다시 키면 로그인을 다시 해야 하는 경우
2. 영속 쿠키 : 만료 날짜를 입력하면 해당 날짜까지 쿠키가 유지된다.
   - Set-Cookie: expires=Sat, 26-Dec-2020 04:39:21 GMT -> GMT 기준으로 입력해야 하며, 만료일이 지나면 쿠키가 자동으로 삭제된다.
   - Set-Cookie: max-age = 3600(초) -> 초 단위로 쿠키의 유지 시간을 설정할 수 있으며, 0 또는 음수를 지정할 경우 쿠키가 삭제된다.
### 쿠키 도메인
![쿠키 도메인](https://github.com/whxogus215/Backend-Study-Archive/assets/70999462/9ae7d34c-49b7-40a3-bde0-6ef5d56b8b7e)
### 쿠키 경로
![쿠키 경로](https://github.com/whxogus215/Backend-Study-Archive/assets/70999462/6d62283a-a0fb-4913-b678-a994b6a4aced)
### 쿠키 보안
![쿠키 보안](https://github.com/whxogus215/Backend-Study-Archive/assets/70999462/c1bf0f51-e2e0-4119-8f96-267744557345)
- 보안 항목에 설정된 조건에 맞게 클라이언트에서 서버 측으로 쿠키를 보낼지 말지를 결정한다.
> [인프런 김영한 HTTP 강좌](https://www.inflearn.com/course/http-%EC%9B%B9-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC/dashboard)

# HTTP 헤더2

## 캐시
캐시는 서버에서 보낸 데이터를 클라이언트의 웹 브라우저에 잠시 저장하는 것을 말한다. 캐시에 저장되는 시간을 서버 측에서 세팅하면
클라이언트는 정해진 시간 안에 같은 요청을 했을 때, 서버로부터 데이터를 받아오지 않고, 웹 브라우저의 캐시 데이터를 사용한다.  

네트워크의 다운로드 속도는 컴퓨터의 메모리 접근 속도에 비해 매우 느리다. 따라서 캐시를 적절하게 사용하면 리소스 접근 시간을 많이 단축시킬 수 있다.

![캐시 적용](https://github.com/whxogus215/Backend-Study-Archive/assets/70999462/cb07f795-5277-4ad1-8760-c5b6c529f586)
> [인프런 김영한 HTTP 강좌](https://www.inflearn.com/course/http-%EC%9B%B9-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC/dashboard)

서버에서 데이터가 변경되지 않았더라도, 쿠키 적용시간이 만료되었다면 서버는 네트워크를 통해 다시 데이터를 보내줘야 한다.
이는 비효율적이며 추가로 시간이 소요된다. 이를 해결하기 위한 방법이 검증헤더와 조건부 요청이다.

## 검증 헤더 (서버에서 보내는 헤더)
서버 측에서 데이터의 최종 수정일을 알려주고, 클라이언트가 요청하는 최종 수정일과 비교해서 일치하면 값이 변경되지 않은 것이다.
따라서 서버에서는 처음에 데이터를 전송함과 동시에 응답 헤더에 `Last-Modified`라는 검증 헤더를 같이 보낸다.
이 검증헤더에는 최종 수정일 날짜 정보가 담겨있다.
![검증헤더1](https://github.com/whxogus215/Backend-Study-Archive/assets/70999462/8c2cd771-7c7c-431e-8e92-12c4cdbc2c30)

검증 헤더의 또다른 종류인 `ETag(Entity Tag)`는 데이터의 수정 날짜가 아니라 데이터의 고유한 버전 데이터를 담고있다.
즉, 데이터가 수정되면 이름도 변경되는 **HashCode를 사용한다.** 만약 데이터가 수정되지 않았다면 HashCode가 그대로인 것이다.
즉, ETag 값을 보내서 데이터가 변경되지 않았으면 값이 같을 것이고, 다를 경우 데이터가 변경되었다는 뜻이므로 데이터를 다시 받아야 하는 것이다.
![검증헤더3](https://github.com/whxogus215/Backend-Study-Archive/assets/70999462/00641452-47f2-4661-837b-51029183764d)

## 조건부 요청 헤더 (클라이언트에서 보내는 헤더)
캐시 시간이 초과하지 않았더라면, 클라이언트는 웹 브라우저의 캐시 메모리에서 데이터를 가져오면 된다. 하지만
시간이 초과했을 때, 캐시 메모리를 사용할 수 없으므로 클라이언트는 서버에게 다시 요청을 보낸다. 이 때,
캐시 메모리에 저장된 최종 수정일(서버 측으로부터 받은 `Last-Modified` 검증헤더 정보) 데이터를 가져와 요청 헤더로 만들어 보낸다.
이 요청헤더를 `If-modified-since`라고 한다. 즉, 최종 수정일 정보를 넘김으로써 서버 측에서 변경된 시간과 일치하는지 보내는 것이다.
  
만약 서버 측에서 전달받은 `If-modified-since` 값과 `Last-Modified` 값이 일치하면, 응답 코드로 ``304 Not Modified``를 보낸다.
...
만약 서버 측에서 ETag 값이 일치한다면, `If-modified-since`와 마찬가지로 **데이터 바디를 보내지 않고 헤더만 보낸다.** 질문이
일치하지 않냐는(If-None-Match) 부정문이기 때문에, 값이 일치한다면 참이 아닌 것이다. 따라서 200 코드를 보내지 않는다. 반대로 값이 일치하지 않으면 데이터가 변경되었다는 것으로
질문에 대한 답이 참이다. 따라서 **200 코드를 보낸다.** (부정문이라 헷갈릴 수 있음)

## 검증 헤더와 조건부 요청 헤더의 조합
### If-Modified-Since : 이후에(Since) 데이터가 수정되었는지(Modified)? + Last-Modified
- 만약 데이터가 변경되지 않았을 경우 : `If-Modified-Since`에 대한 값이 참이 아니다. 따라서 **200 코드를 전송하지 않는다.**
  - **304 Not Modified(Redirect)**, 헤더 데이터만 전송하게 된다.
  - 전송 용량 : 헤더 용량
- 만약 데이터가 변경되었을 경우 : `If-Modified-Since`에 대한 값이 참이다. 따라서 **200 코드를 전송한다.**
  - **200 OK**, 모든 데이터를 전송한다.
  - 전송 용량 : 헤더 용량 + **바디 용량**
- 단점 : 검증 헤더의 날짜와 조건부 요청 헤더의 날짜가 다르더라도, 데이터가 수정이 안되었을 수도 있다.(데이터 원상복구, 주석/스페이스 등등)
따라서 서버 측에서 캐시 로직을 컨트롤 할 수 있는 방법이 있다. 바로 **ETag와 If-None-Match 조합이다.**

### If-None-Match : ETag 값이 일치하지 않는지? + ETag
- 만약 데이터가 변경되지 않았을 경우 : `If-None-Match` 값이 참이 아니다. (일치하기 때문) 따라서 **200 코드를 전송하지 않는다.**
  - **304 Not Modified**, 헤더 데이터만 전송하게 된다.
  - 전송 용량 : 헤더 용량
- 만약 데이터가 변경되었을 경우 : `If-None-Match` 값이 참이다. (값이 변경되어 ETag 값이 다르기 때문) 따라서 **200 코드를 전송한다.**
  - **200 OK**, 모든 데이터를 전송한다.
  - 전송 용량 : 헤더 용량 + **바디 용량**
- 장점 : If-Modiifed-Since에 비해 **캐시 제어 로직을 서버에서 완전히 관리한다.** 즉, 클라이언트는 단순히 ETag 값만 넘겨주면 된다.
예를 들어, 애플리케이션의 배포 주기에 맞춰서 서버 측에서 ETag를 갱신한다면, 사용자는 배포가 새로 될 때마다 데이터를 다시 내려받게 된다. 

## Cache-Control 헤더
서버 측에서 캐시에 대한 정보를 보내는 여러 정보들이 있다.
- Cache-Control : max-age
  - 이전에는 날짜 단위(Expires)로 저장했지만, 보다 유연한 방법으로 초단위로 설정한다. **따라서 캐시의 유효 시간은 초 단위로 설정된다.**
- Cache-Control : no-cache
  - **no cache라고 해서 캐시를 사용하지 말라는 뜻이 아니다.** 캐시를 사용하되, 사용할 때마다 Origin 서버(프록시 서버 X)로부터 항상 검증을 받고 사용하라는 뜻이다.
    - 검증 방법 : `If-Modified-Since` or `If-None-Match`
- Cache-Control : no-store
  - no store라는 것은 저장하지 말라는 뜻이다. 즉, **캐시에 데이터를 저장하지 말라는 뜻이다.** 오히려 캐시를 사용하지 말라는 측면에서 `no-cache`보다 더 가깝다.
...
`no-cache`는 프록시 캐시에서 데이터를 가져올 수 있는 위험이 있기 때문에 이를 방지하기 위해 `must-revalidate`를 같이 써준 것이라고 이해해도 될 것 같다.
어쨌든 캐시를 사용하지 않고 계속 서버로부터 데이터를 가져와야 하기 때문에 `no-store`도 같이 있어야 한다.
